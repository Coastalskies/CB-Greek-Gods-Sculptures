...
The Greek Gods & Deities project is hosted on GitHub, utilizing CollectionBuilder and a CSV file from Google Sheets. The objects are sourced from a variety of museums in Europe and the United States. CollectionBuilder is a template for GitHub Pages that allows you to create a collection of images and data. This collection, in particular, focuses on ancient Greek statues and busts, with an emphasis on depictions of prominent mythological figures, including relevant Greek Gods. These works can be dated from the Archaic period (c. 500 BCE) through the Hellenistic period (as late as 10 BCE), while also including some pieces that transition into early Roman times (27 BCE–14 CE).
